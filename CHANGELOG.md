...
* The recording list now shows the duration of each item, even for those not generated by BCR (#235).

* The behavior of the Prev/Next buttons now mimics that of other media players (#230):
  * they are always enabled
  * Prev will skip to the previous recording if the playback position is <= 3 seconds; otherwise, it will reset the current position to 0

Bug fixes:

* `.amr` playback position can't be set if they are VBR and/or miss a proper header
